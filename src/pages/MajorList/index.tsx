...
import {getMajorList, getSchoolList, isTrailUser} from "@/services/ant-design-pro/api";
  ...
  return <PageContainer>
    ...
    <Card hidden={!isTrail}>
      ...
      <div>
        ...
        {
          majorList.map(({province, majorInfoList}) => {
            console.info("value is ", province);
            return <Card style={{ marginTop: 10 }}>
              <ProDescriptions column={3} title={province}>
                {
                  majorInfoList.map(value => {
                    if (isTrail) {
                      if(value.vip){
                        return <li><a
                            href={'#'} disabled="disabled">{value.majorName}(仅付费用户可查看)</a>
                        </li>
                      }else{
                        return <li><a href={'/major_detail/' + value.id}>{value.schoolName+'-'+ value.departmentName +'-'+ value.majorName}(试用用户可查看)</a></li>
                      }
                    } else {
                      return <li><a href={'/major_detail/' + value.id}>{value.schoolName+'-'+ value.departmentName +'-'+ value.majorName}</a></li>
                    }
                  }
                  )
                }
