...
import {getEnrollmentRankList, getRankList, getSchoolList} from "@/services/ant-design-pro/api";
...
const Page: React.FC = () => {
  ...
  useEffect( () => {
    getEnrollmentRankList().then(({data})=>{
      console.info(data);
      setRankList(data);
    });
  },[]);
              ...
              <ProDescriptions column={1} title={rankType}>
                {
                  subRankList.map(value => {
                    console.info(value)
                    return <li><a href={'/enrollment_rank_page/' + value.rankName}>{value.rankName}</a></li>
                    }
                  )
                }
              </ProDescriptions>
